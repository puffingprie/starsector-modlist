...
public class acs_raiderdriveStats extends BaseShipSystemScript {
    //start damper
		...
		if (stats.getEntity() instanceof ShipAPI) {
			...
			ship.getEngineController().extendFlame(this, 2f * effectLevel, 0f * effectLevel, 0f * effectLevel);
							...
							if (ship != engine.getPlayerShip() || (ship == engine.getPlayerShip() && engine.getPlayerShip().getAI() != null)) {
								ship.giveCommand(ShipCommand.ACCELERATE_BACKWARDS, ship, 1);
							}

							

							// if (dataTimer.runOnces) {
							// 	if (ship != engine.getPlayerShip() || (ship == engine.getPlayerShip() && engine.getPlayerShip().getAI() != null)) {
							// 		ship.giveCommand(ShipCommand.ACCELERATE_BACKWARDS, ship, 1);
							// 	}
							// }
							...
							if (dataTimer.interval.intervalElapsed()) {
								for (WeaponAPI weapon : ship.getAllWeapons()) {
									...
									ship.giveCommand(ShipCommand.DECELERATE, ship, 1);

...
public class dpl_song_of_dimensionChargeGlow extends CombatEntityPluginWithParticles {
	...
	public static int MAX_ARC_RANGE = 150;
	public static int REAL_DAMAGE = 60;
	//public static int ARCS_ON_HIT = 15;
	
	public static float REPAIR_RATE_MULT = 0.4f;
	...
	public static class RDRepairRateDebuff implements AdvanceableListener {
		public static String DEBUFF_ID = "song_of_dimension_repair_debuff";
		...
		public void advance(float amount) {
			...
			if (Global.getCurrentState() == GameState.COMBAT &&
				...
				Global.getCombatEngine().maintainStatusForPlayerShip(STATUS_KEY,
						...
						"SONG OF DIMENSION", "SLOWER REPAIRS: " + (int)Math.max(1, Math.round(dur)) + " SEC", true);
			}
...
//		}
	...
	public void spawnArc() {
		...
		if (target != null) {
			EmpArcEntityAPI arc = engine.spawnEmpArcPierceShields(proj.getSource(), proj.getLocation(), null,
					   target,
					   DamageType.ENERGY, 
					   dam,
					   emp, // emp 
					   100000f, // max range 
					   "song_of_dimension_emp_impact",
					   thickness, // thickness
					   color,
			...
			if (target instanceof ShipAPI) {
				...
				engine.addFloatingDamageText(arc.getTargetLocation(), REAL_DAMAGE, Misc.FLOATY_HULL_DAMAGE_COLOR, s, proj.getSource());
				if (s.getHitpoints() <= 10) {
					//Just kill it if its hull point is too low.
					engine.applyDamage(s, s.getLocation(), 1000*dam, DamageType.HIGH_EXPLOSIVE, 0f, true, false, null);
				}

...
//	}
	...
	protected void activateImpl() {
		...
		for (SectorEntityToken comm_relay:comm_relays) {
			...
			FactionAPI faction = comm_relay.getFaction();
			if (faction == null) continue;
			if (faction.getId() == "dpl_phase_lab") continue;
			if (faction.isPlayerFaction()) continue;
			if (faction.isHostileTo("dpl_phase_lab")) continue;
			
			Global.getSector().getMemoryWithoutUpdate().set("$dpl_ScrewedUp", true);
			Color highlightColor = Misc.getNegativeHighlightColor();
			float curRel = faction.getRelationship("dpl_phase_lab");
			faction.setRelationship("dpl_phase_lab", curRel - 0.05f);
			Global.getSector().getCampaignUI().addMessage("Due to suspicions, the relation between the Phase Lab and " + faction.getDisplayName() + " has been lowered by 5.", highlightColor);
		}
...
//	}
	...
	public void createTooltip(TooltipMakerAPI tooltip, boolean expanded) {
		...
		if (!isUsable() && super.isUsable() && getFleet() != null) {
			tooltip.addPara("You need Elly in your fleet and a comm relay in the system.", bad, pad);
		}

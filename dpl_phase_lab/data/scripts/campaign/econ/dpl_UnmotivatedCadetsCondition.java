...
public class dpl_UnmotivatedCadetsCondition extends BaseHazardCondition {
	public int modifier;
	...
	public void apply(String id) {
		...
		if (industry == null) {
			if (Industries.MILITARYBASE.equals(industryId)) {
				List<Industry> AllIndustries = market.getIndustries();
				for (Industry the_industry : AllIndustries) {
					if (the_industry.getSpec().hasTag(Industries.TAG_COMMAND) || the_industry.getSpec().hasTag(Industries.TAG_MILITARY)) {
		...
		if (Admin != null) {
			int level = (int) Admin.getStats().getSkillLevel(Skills.INDUSTRIAL_PLANNING);
			if (level > 0) {
				hasAdmin = true;
			}
		}

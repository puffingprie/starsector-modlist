...
public class dpl_MoteScript extends BaseShipSystemScript {
	
	protected static float MAX_ATTRACTOR_RANGE = 1000f;
	public static float MAX_DIST_FROM_SOURCE_TO_ENGAGE_AS_PD = 1200f;
	public static float MAX_DIST_FROM_ATTRACTOR_TO_ENGAGE_AS_PD = 1000f;
	
	public static int MAX_MOTES_HF = 20;
...
//	}
	...
	public static String MOTELAUNCHER_HF = "dpl_hydraulis_notes";
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (launchInterval.intervalElapsed()) {
			...
			if (data.motes.size() < maxMotes && data.attractorLock == null &&// false && 
				...
				String weaponId = "dpl_hydraulis_notes";
		...
		if (effectLevel == 1) {
			...
			if (player) {
				...
				ShipAPI shipTarget = Misc.findClosestShipEnemyOf(ship, ship.getLocation(), HullSize.FRIGATE, MAX_ATTRACTOR_RANGE, true);
				if (shipTarget != null) {
					Vector2f targetLoc = shipTarget.getLocation();
					Vector2f shipLoc = ship.getLocation();
					float dist = Misc.getDistance(shipLoc, targetLoc);
					isClose = (dist <= MAX_ATTRACTOR_RANGE);
				}
				if (isClose) {
					data.attractorLock = shipTarget;
					data.attractorTarget = shipTarget.getLocation();
					target = data.attractorLock;
				} else {
					target = asteroid;
				}
			...
			EmpArcEntityAPI arc = (EmpArcEntityAPI)engine.spawnEmpArc(ship, slotLoc, ship, target,
							   ...
							   dpl_MoteScript.getEMPColor(ship),
...
//			}
	...
	protected void spawnAttractorParticles(ShipAPI ship) {
		...
		float minSize = 15f;
		float maxSize = 30f;
		Color color = dpl_MoteScript.getEMPColor(ship);
		for (int i = 0; i < glows; i++) {
			float radius = minRadius + (float) Math.random() * (maxRadius - minRadius);
			Vector2f loc = Misc.getPointAtRadius(targetLoc, radius);
			Vector2f dir = Misc.getUnitVectorAtDegreeAngle(Misc.getAngleInDegrees(loc, targetLoc));
	...
	public void calculateTargetData(ShipAPI ship) {
		...
		if (data.attractorLock != null) {
			...
			Global.getCombatEngine().addPlugin(createTargetJitterPlugin(data.attractorLock,
					...
					dpl_MoteScript.getJitterColor(ship)));
		}
	}

...
public class dpl_MoteStrikeStats extends BaseShipSystemScript implements MineStrikeStatsAIInfoProvider {
	...
	//Find lightning slot
	protected WeaponSlotAPI mote_pointer = null;
	
	protected void findSlots(ShipAPI ship) {
		if (mote_pointer != null) return;
	...
	public void spawnMine(ShipAPI source, Vector2f mineLoc) {
		...
		Vector2f slotLoc = mote_pointer.computePosition(source);
		
		float thickness = 60f;
		float coreWidthMult = 0.67f;
		EmpArcEntityAPI arc = engine.spawnEmpArc(source, slotLoc, source,
					   mine,
					   DamageType.ENERGY, 
					   0f, // dam 
					   0f, // emp 
					   100000f, // max range 
					   "mote_attractor_targeted_ship", //weapon sound id
					   thickness, // thickness
					   new Color(100,100,175,255), //fringe color
					   new Color(150,150,255,200) //core color
					   );
...
//			mine.getDamage().setMultiplier(mine.getDamage().getMultiplier() * extraDamageMult);
		...
		mine.setWeaponSpec("dpl_phase_lights");
		mine.getVelocity().scale(0);
		mine.setEmpResistance(10000);

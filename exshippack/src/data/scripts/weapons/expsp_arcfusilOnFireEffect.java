...
/**
 */
public class expsp_arcfusilOnFireEffect implements OnFireEffectPlugin {
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		if (target != null) {
			...
			int arcNum= getRandomNumberInRange(1,3);
			float emp2 = projectile.getEmpAmount()/5;
			float dam2 = projectile.getDamageAmount()/5;
			for(int arcsFired=0; arcsFired < arcNum; arcsFired++ ) {

				engine.spawnEmpArcPierceShields(projectile.getSource(), arc.getTargetLocation(), weapon.getShip(),target,
						DamageType.ENERGY,
						dam2,
						emp2, // emp
						100000f, // max range

...
public class expsp_holorifleHit extends BaseCombatLayeredRenderingPlugin implements OnHitEffectPlugin {

	// each tick is on average .9 seconds
	// ticks can't be longer than a second or floating damage numbers separate
	public static int NUM_TICKS = 5;
	public static float TOTAL_DAMAGE = 150;
	
	public expsp_holorifleHit() {
	}
	...
	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
		...
		expsp_holorifleHit effect = new expsp_holorifleHit(projectile, (ShipAPI) target, offset);
	...
	public static class ParticleData {
		...
		public ParticleData(float baseSize, float maxDur, float endSizeMult) {
			//sprite = Global.getSettings().getSprite("fx", "hit_fx_pixel");
	...
	public expsp_holorifleHit(DamagingProjectileAPI proj, ShipAPI target, Vector2f offset) {
		this.proj = proj;
		this.target = target;
		this.offset = offset;
	...
	public void render(CombatEngineLayers layer, ViewportAPI viewport) {
		...
		Color effectcolor = new Color(212,175,55,235);

...
public class expsp_divineLight extends BaseShipSystemScript {
	...
	public static final Color JITTER_UNDER_COLOR = new Color(55,100,250,105);
	public static final Color JITTER_COLOR = new Color(55,100,250,55);
...
//				fStats.getMissileWeaponDamageMult().modifyPercent(id, DAMAGE_INCREASE_PERCENT * effectLevel);
				...
				//fStats.getTimeMult().modifyMult(id, 1f+0.01f*SPEED_INCREASE_PERCENT*effectLevel);
...
//		}
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		if (index == 0) {
			...
			return new StatusData("" + Misc.getRoundedValueMaxOneAfterDecimal(DAMAGE_INCREASE_PERCENT * effectLevel) + "% fighter damage buff", false);
		} else if (index == 1) {
			//return new StatusData("+" + (int) damageTakenPercent + "% weapon/engine damage taken", false);
			return new StatusData("" + Misc.getRoundedValueMaxOneAfterDecimal(SPEED_INCREASE_PERCENT * effectLevel) + "% fighter speed and damage resistance buff" , false);

		}else if (index==2){
			return new StatusData("Max speed increased by 20" , false);
		}
		return null;
	}

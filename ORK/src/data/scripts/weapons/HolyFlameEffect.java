 ...
 */
public class HolyFlameEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {

	public HolyFlameEffect() {
	}
	
	protected List<HolyFlameEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<HolyFlameEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (HolyFlameEffect curr : trails) {
				if (curr.proj != null) {
			...
			if (weight > 0.1f) {
				...
				Global.getSoundPlayer().playLoop("holy_cleanse_loop", ship, 1f, volume, com, ship.getVelocity());
			}
		}
		...
		for (int i = 0; i < numIter; i++) { 
			for (HolyFlameEffect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		HolyFlameEffect trail = new HolyFlameEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<HolyFlameEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public HolyFlameEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;

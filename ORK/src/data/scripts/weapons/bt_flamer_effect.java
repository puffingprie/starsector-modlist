 ...
 */
public class bt_flamer_effect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {

	public bt_flamer_effect() {
	}
	
	protected List<bt_flamer_effect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<bt_flamer_effect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (bt_flamer_effect curr : trails) {
				if (curr.proj != null) {
			...
			if (weight > 0.1f) {
				...
				Global.getSoundPlayer().playLoop("plasma_shotgun_shot", ship, 1f, volume, com, ship.getVelocity());
			}
		}
		...
		for (int i = 0; i < numIter; i++) { 
			for (bt_flamer_effect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		bt_flamer_effect trail = new bt_flamer_effect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<bt_flamer_effect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public bt_flamer_effect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;

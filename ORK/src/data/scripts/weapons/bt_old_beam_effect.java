...
{
	IntervalUtil interval = new IntervalUtil(0.15f, 0.15f); // min/max duration between arcs, in seconds
	final float ARC_DAMAGE_AMOUNT = 300f; // damage type will match the beam's damage type
	...
	final float ARC_DISTANCE = 0.25f;
		...
		if (target instanceof ShipAPI && beam.getBrightness() >= 1f)
		{
			...
			ShipAPI shipTarget = (ShipAPI) target;
				...
				if (!shipTarget.hasListenerOfClass(bt_shieldinterference.class)) {
					...
					//log.info("added beam listener to ship");
				}
				...
				if (listener.stacks.size() < 50) {
					listener.stacks.add(new bt_shieldinterference.InterferenceStack(6f)); //100 stacks = max 50%
					//log.info("added a new stack to the interference");
				}
				return;
			}

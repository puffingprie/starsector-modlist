...
public class SpaceShuntStats extends BaseShipSystemScript {
			...
			ship.getEngineController().extendFlame(this, 2f * effectLevel, 0f * effectLevel, 0f * effectLevel);
                            ...
                            for (int x = 0; x < 30; x++) {
                                Global.getCombatEngine().spawnEmpArc(ship, ship.getLocation(),
                                                   empTarget,
                                                   empTarget, DamageType.ENERGY, 10, 25,
                                                   2000, null, 30f, new Color(230,40,40,180),
                                                   new Color(255,255,255,255));
                            }	

                            for (CombatEntityAPI EmpTarget :AIUtils.getNearbyEnemies(ship,600)) {
                                for (int x = 0; x < 1; x++) {
                                
                                Global.getCombatEngine().spawnEmpArc(ship, ship.getLocation(),
                                                   EmpTarget,
                                                   EmpTarget, DamageType.ENERGY, 1200, 1200,
                                                   60000, null, 60f, new Color(230,40,40,255),
                                                   new Color(255,255,255,255));

                                }
                            }
                               
                            for (CombatEntityAPI EmpTarget :AIUtils.getNearbyAllies(ship,600)) {
                                for (int x = 0; x < 1; x++) {
                                
                                Global.getCombatEngine().spawnEmpArc(ship, ship.getLocation(),
                                                   EmpTarget,
                                                   EmpTarget, DamageType.ENERGY, 100, 800,
                                                   30000, null, 60f, new Color(230,40,40,255),
                                                   new Color(255,255,255,255));
                                }
                            }

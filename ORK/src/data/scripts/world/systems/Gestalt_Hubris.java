...
public class Gestalt_Hubris{
    ...
    public void generate(SectorAPI sector) {
        ...
        StarSystemAPI system = sector.createStarSystem("Hubris");
        system.getLocation().set(-10300, -42500);
        system.setBackgroundTextureFilename("graphics/backgrounds/background_hubris.jpg");
        system.getMemoryWithoutUpdate().set(MusicPlayerPluginImpl.MUSIC_SET_MEM_KEY, "music_gestalt_ambience");


        // create the star and generate the hyperspace anchor for this system
        PlanetAPI GestaltHubris = system.initStar("bt_hubris", // unique id for this star
                "black_hole", // id in planets.json
                340f,        // radius (in pixels at default zoom)
                750f, // corona radius, from star edge
        ...
        float orbitRadius5 = GestaltHubris.getRadius() * 3f;
        ...
        for (float i = 0; i < numBands5; i++) {
            ...
            RingBandAPI visual = system.addRingBand(GestaltHubris, "misc", ring, 80f, 1, new Color(101, 49, 49, 255), bandWidth5,
                    radius + bandWidth5 + 50 / 2f, -orbitDays);
            RingBandAPI visual2 = system.addRingBand(GestaltHubris, "misc", ring, 80f, 2, new Color(101, 49, 49, 255), bandWidth5,
        ...
        float orbitRadius1 = GestaltHubris.getRadius() * 16f;
        ...
        for (float i = 0; i < numBands1; i++) {
            ...
            String ring = rings.pick();
            RingBandAPI visual = system.addRingBand(GestaltHubris, "misc", ring, 256f, 0, new Color(173, 35, 35), bandWidth1,
                    radius + bandWidth1 / 2f, -orbitDays);
            RingBandAPI visual2 = system.addRingBand(GestaltHubris, "misc", ring, 256f, 1, new Color(173, 35, 35), bandWidth1,
                    radius + bandWidth1 / 2f, -orbitDays);
            RingBandAPI visual3 = system.addRingBand(GestaltHubris, "misc", ring, 256f, 2, new Color(173, 35, 35), bandWidth1,
                    radius + bandWidth1 / 2f, -orbitDays);
            float spiralFactor = 2f + Misc.random.nextFloat() * 5f;
            visual.setSpiral(true);
            visual.setMinSpiralRadius(GestaltHubris.getRadius() / 3);
            visual.setSpiralFactor(spiralFactor);
            visual2.setSpiral(true);
            visual2.setMinSpiralRadius(GestaltHubris.getRadius() / 2);
            visual2.setSpiralFactor(spiralFactor);
            visual3.setSpiral(true);
            visual3.setMinSpiralRadius(GestaltHubris.getRadius());
            visual3.setSpiralFactor(spiralFactor);

        }


        float orbitRadius4 = GestaltHubris.getRadius() * 14f;
        ...
        for (float i = 0; i < numBands4; i++) {
            ...
            rings.add("rings_asteroids0");
            String ring = rings.pick();
            RingBandAPI visual = system.addRingBand(GestaltHubris, "misc", ring, 130f, 1, Color.red, 130,
                    radius + bandWidth4 / 2f, -orbitDays);
            RingBandAPI visual2 = system.addRingBand(GestaltHubris, "misc", ring, 130f, 2, Color.red, 130,
                    radius + bandWidth4 / 2f, -orbitDays);
            RingBandAPI visual3 = system.addRingBand(GestaltHubris, "misc", ring, 100f, 1, Color.red, 130,
                    radius + bandWidth4 / 2f, -orbitDays);
            float spiralFactor = 2f + Misc.random.nextFloat() * 5f;
            visual.setSpiral(true);
            visual.setMinSpiralRadius(GestaltHubris.getRadius() / 2f);
            visual.setSpiralFactor(spiralFactor);
            visual2.setSpiral(true);
            visual2.setMinSpiralRadius(GestaltHubris.getRadius() / 2f);
            visual2.setSpiralFactor(spiralFactor);
            visual3.setSpiral(true);
            visual3.setMinSpiralRadius(GestaltHubris.getRadius() / 2f);
            visual3.setSpiralFactor(spiralFactor);
        }


        float orbitRadius3 = GestaltHubris.getRadius() * 10f;
        ...
        for (float i = 0; i < numBands3; i++) {
            ...
            rings.add("rings_dust0");
            String ring = rings.pick();
            RingBandAPI visual = system.addRingBand(GestaltHubris, "misc", ring, 256f, 2, Color.red, bandWidth3,
        ...
        float orbitRadius2 = GestaltHubris.getRadius() * 5f;
        ...
        for (float i = 0; i < numBands2; i++) {
            ...
            rings.add("rings_dust0");
            String ring = rings.pick();
            RingBandAPI visual = system.addRingBand(GestaltHubris, "misc", ring, 256f, 3, Color.red, bandWidth2,
        ...
        SectorEntityToken ring = system.addTerrain(Terrain.RING, new BaseRingTerrain.RingParams(orbitRadius1, orbitRadius1 / 2f, GestaltHubris, "Accretion Disk"));
        ring.addTag(Tags.ACCRETION_DISK);
        ring.setCircularOrbit(GestaltHubris, 0, 0, -100);


        PlanetAPI Ardamas = system.addPlanet("bt_planet_Ardamas",
                GestaltHubris,
                "Ard-amas",
                "irradiated",
                0,
        ...
        //Abandoned Station
        SectorEntityToken neutralStation = system.addCustomEntity("bt_Hubris_AbandonedStation", "Abandoned Station", "station_side05", "neutral");
        neutralStation.setCircularOrbitPointingDown(Ardamas, 0, 600, syncOrbitDays);
        ...
        neutralStation.setCustomDescriptionId("bt_Hubris_AbandonedStation");
        neutralStation.setInteractionImage("illustrations", "abandoned_station3");
        MarketAPI market = Global.getFactory().createMarket("bt_Hubris_AbandonedStation", "Abandoned Station", 0);
        ...
        neutralStation.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addSupplies(MathUtils.getRandomNumberInRange(230, 400));
        neutralStation.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addCommodity(Commodities.ORGANS, MathUtils.getRandomNumberInRange(345, 640));
        ...
        ResearchStation.setCircularOrbit(GestaltHubris, 180, 2000, syncOrbitDays);

        addDerelict(system, GestaltHubris, "ork_battlecruiser_aggressive", ShipRecoverySpecial.ShipCondition.WRECKED, 800 + ((float) Math.random() * 200f), 0, (Math.random() < 0.4));
        addDerelict(system, GestaltHubris, "ork_vencha_support", ShipRecoverySpecial.ShipCondition.WRECKED, 800 + ((float) Math.random() * 200f), 120, (Math.random() < 0.4));
        addDerelict(system, GestaltHubris, "ork_tail_standard", ShipRecoverySpecial.ShipCondition.WRECKED, 800 + ((float) Math.random() * 200f), 240, (Math.random() < 0.4));

        addDerelict(system, GestaltHubris, "ork_faster_elite", ShipRecoverySpecial.ShipCondition.BATTERED, 1500 + ((float) Math.random() * 200f), 0, (Math.random() < 0.4));
        addDerelict(system, GestaltHubris, "ork_crusher_standard", ShipRecoverySpecial.ShipCondition.BATTERED, 1500 + ((float) Math.random() * 200f), 120, (Math.random() < 0.4));
        addDerelict(system, GestaltHubris, "ork_bees_standard", ShipRecoverySpecial.ShipCondition.BATTERED, 1500 + ((float) Math.random() * 200f), 240, (Math.random() < 0.4));

        addDerelict(system, GestaltHubris, "ork_faster_standard", ShipRecoverySpecial.ShipCondition.BATTERED, 2200 + ((float) Math.random() * 200f), 0, (Math.random() < 0.4));
        addDerelict(system, GestaltHubris, "ork_skrunkly_standard", ShipRecoverySpecial.ShipCondition.BATTERED, 2200 + ((float) Math.random() * 200f), 120, (Math.random() < 0.4));
        addDerelict(system, GestaltHubris, "ork_speeder_standard", ShipRecoverySpecial.ShipCondition.BATTERED, 2200 + ((float) Math.random() * 200f), 240, (Math.random() < 0.4));

        generateNebula(system);

        system.setLightColor(new Color(255, 25, 25, 136)); // light color in entire system, affects all entities

        // generates hyperspace destinations for in-system jump points
        system.generateAnchorIfNeeded();
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(Ardamas, 0f);
        well.addTag(Tags.NO_ENTITY_TOOLTIP);
        well.setColorOverride(new Color(255, 0, 0));
        hyper.addEntity(well);
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(Ardamas, 10);
        /*
    ...
    protected void generateNebula(StarSystemAPI system) {
        ...
        nebula.setTerrainName("Regona Nebula");

...
public class GigaRail_PiercingProjectileBehaviour extends BaseEveryFrameCombatPlugin {
	
	private static final Color COLOR = new Color(255, 255, 255, 0);
	private final float DAMAGE_MODIFIER = 0.65f; 	//A factor for adjusting damage based on distance and rate of occurance.
	private final float BASE_SPEED = 4000f;
			...
			for (DamagingProjectileAPI Proj : engine.getProjectiles()){
				if (Proj != null){
					if (Proj.getCollisionClass() == CollisionClass.NONE && Proj.getWeapon() != null && Proj.getWeapon().getSpec() != null && !Proj.isFading()){
						...
						//THE GUN
						if (Proj.getWeapon().getSpec().hasTag("giga_rail")){
							DamagingExplosionSpec PierceEffect1 = CalculateDamage(Damage,Speed,50f);
							DamagingExplosionSpec PierceEffect2 = CalculateDamage(Damage,Speed,50f);
							DamagingExplosionSpec PierceEffect3 = CalculateDamage(Damage,Speed,50f);
							DamagingExplosionSpec PierceEffect4 = CalculateDamage(Damage,Speed,50f);

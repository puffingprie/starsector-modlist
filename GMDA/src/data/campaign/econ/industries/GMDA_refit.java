...
public class GMDA_refit extends BaseIndustry implements RouteManager.RouteFleetSpawner, FleetEventListener {
		...
		super.apply(true);
				...
				supply(1, Commodities.SHIPS, shipBonus, "GMDA Refit Workshop");

				Pair<String, Integer> deficit = getMaxDeficit(Commodities.RARE_METALS, Commodities.HEAVY_MACHINERY);
				int maxDeficit = size -3;
				if (deficit.two > maxDeficit) deficit.two = maxDeficit;
				applyDeficitToProduction(2, deficit, Commodities.SHIPS);

				market.getStats().getDynamic().getMod(Stats.PRODUCTION_QUALITY_MOD).modifyFlat(getModId(1), qualityBonus, "GMDA Refit Workshop");
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		params.flagshipVariantId = "onslaught_GM_elite";
		...
		fleet.getFlagship().setId("onslaught_GM_elite");
		fleet.addScript(new GMDAFleetManager.CowardChange(fleet));

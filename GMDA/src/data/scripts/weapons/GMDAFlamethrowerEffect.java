 ...
 */
public class GMDAFlamethrowerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {
	public GMDAFlamethrowerEffect() {
	}
	
	protected List<GMDAFlamethrowerEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<GMDAFlamethrowerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (GMDAFlamethrowerEffect curr : trails) {
				if (curr.proj != null) {
		...
		for (int i = 0; i < numIter; i++) { 
			for (GMDAFlamethrowerEffect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		GMDAFlamethrowerEffect trail = new GMDAFlamethrowerEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<GMDAFlamethrowerEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public GMDAFlamethrowerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;

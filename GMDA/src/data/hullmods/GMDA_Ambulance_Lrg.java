...
public class GMDA_Ambulance_Lrg implements HullModEffect, HullModFleetEffect {
    ...
    public void onFleetSync(CampaignFleetAPI fleet) {
        ...
        for (FleetMemberAPI rig : rigs) {
            for (int i = index; i < members.size(); i++) {
                ...
                member.getStats().getDynamic().getStat(Stats.FIGHTER_CREW_LOSS_MULT).modifyMult(STAT_MOD_ID, CREW_SAVE_BONUS_LRG);
				member.getStats().getCrewLossMult().modifyMult(STAT_MOD_ID, 1f - CASUALTY_REDUCTION_LRG);
                break;
            }
        }
    }

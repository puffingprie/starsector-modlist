...
///Forked from MagicGuidedProjectileScript

package data.weapons.guidedProject;
...
public class FM_GuidedProjectSrc_RumiaSystem extends BaseEveryFrameCombatPlugin {
    ...
    private static final float TARGET_REACQUIRE_RANGE = 1500f;
    ...
    private static final float TARGET_REACQUIRE_ANGLE = 15f;
    ...
    private static final int INTERCEPT_ITERATIONS = 0;
    ...
    private static final float GUIDANCE_DELAY_MAX = 0.2f;
     ...
     */
    public FM_GuidedProjectSrc_RumiaSystem(@NotNull DamagingProjectileAPI proj, CombatEntityAPI target) {
        this.proj = proj;
        this.target = target;
        ...
        lifeCounter = 0f;
        estimateMaxLife = 1f; // some special adjust to avoid getWeapon()
        delayCounter = 0f;
        ...
        if (GUIDANCE_MODE_PRIMARY.equals("ONE_TURN_DUMB")) {
            // some special adjust to avoid getWeapon().getCurrAngel
            targetAngle = proj.getSource().getFacing() + MathUtils.getRandomNumberInRange(-ONE_TURN_DUMB_INACCURACY, ONE_TURN_DUMB_INACCURACY);

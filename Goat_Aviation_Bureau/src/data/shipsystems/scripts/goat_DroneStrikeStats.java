...
public class goat_DroneStrikeStats extends BaseShipSystemScript implements DroneStrikeStatsAIInfoProvider {
	...
	public static class DroneMissileScript extends BaseCombatLayeredRenderingPlugin {
		...
		public void advance(float amount) {
			...
			if (!engine.isEntityInPlay(missile) || missile.isFizzling() || (missile.getHitpoints() <= 0 && !missile.didDamage()) || droneDestroyed) {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		for (ShipAPI drone : getDrones(ship)) {
			...
			drone.setExplosionFlashColorOverride(new Color(50, 255, 231, 55));
		}
	...
	public void convertDrones(ShipAPI ship, final ShipAPI target) {
		...
		java.util.List<ShipAPI> drones = getDrones(ship);
		if (target != null) {
		...
		for (ShipAPI drone : drones) {
			if (num < getNumToFire()) {
				MissileAPI missile = (MissileAPI)engine.spawnProjectile(ship, weapon, getWeaponId(), new Vector2f(drone.getLocation()), drone.getFacing(), new Vector2f(drone.getVelocity()));
				...
				drone.setWing(null);
				drone.setExplosionFlashColorOverride(new Color(50, 200, 255, 55));
				...
				EmpArcEntityAPI arc = engine.spawnEmpArcVisual(ship.getLocation(), ship, missile.getLocation(), missile, thickness, new Color(100, 214, 255, 155), Color.white);
	...
	protected ShipAPI findTarget(ShipAPI ship) {
		...
		target = Misc.findClosestShipEnemyOf(ship, ship.getMouseTarget(), ShipAPI.HullSize.FIGHTER, Float.MAX_VALUE, true);
		if (target != null && target.isFighter()) {
			ShipAPI nearbyShip = Misc.findClosestShipEnemyOf(ship, target.getLocation(), ShipAPI.HullSize.FRIGATE, 100, false);
	...
	public java.util.List<ShipAPI> getDrones(ShipAPI ship) {
	...
	public boolean isUsable(ShipSystemAPI system, ShipAPI ship) {
		if (ship != null && ship.getSystem() != null && ship.getSystem().getState() != ShipSystemAPI.SystemState.IDLE) {

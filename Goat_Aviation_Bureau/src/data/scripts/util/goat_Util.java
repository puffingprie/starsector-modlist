...
public class goat_Util {
	...
	public static MarketAPI addMarketplace(String factionID, SectorEntityToken primaryEntity, ArrayList<String> marketConditions, ArrayList<String> marketIndustries, ArrayList<String> submarkets, float tarrif, boolean hasCore, boolean hasForge, boolean hasSynchrotron) {
	...
	public static List<EmpArcEntityAPI> visualEmpStorm(Vector2f point, int empAmount, float empMaxLength, float empMinLength, float stormDistance, float stormRandomDistance, CombatEntityAPI anchor, Color colora, Color colorb, boolean sound) {
		if (colora == null) {
		...
		List<EmpArcEntityAPI> list = new ArrayList<>();
		for (int i = 0; i < empAmount; i++) {
			...
			EmpArcEntityAPI arc = Global.getCombatEngine().spawnEmpArcVisual(start, anchor, end, null, 20f + (float)Math.random() * 10f, colora, colorb);
			list.add(arc);
			if (sound) {
	...
	public static RippleDistortion easyRippleIn(Vector2f location, Vector2f velocity, float size, float intensity, float fadesize) {
		return easyRippleIn(location, velocity, size, intensity, fadesize, 60f);
	}

	public static RippleDistortion easyRippleIn(Vector2f location, Vector2f velocity, float size, float intensity, float fadesize, float frameRate) {
		if (intensity == -1f) {
			intensity = size / 4f;
		}
		if (velocity == null) {
	...
	public static float getTimeToAim(WeaponAPI weapon, Vector2f aimAt) {
		...
		if (Math.abs(weapon.distanceFromArc(aimAt)) >= 10f) {
			...
			turnSpeed = Math.max(weapon.getTurnRate(), weapon.getShip().getMutableStats().getMaxTurnRate().getModifiedValue());
			time = Math.abs(MathUtils.getShortestRotation(weapon.getCurrAngle(), VectorUtils.getAngle(weapon.getLocation(), aimAt))) / turnSpeed;
		}
	...
	public static boolean willProjectileHitShipWithInFrame(DamagingProjectileAPI proj, ShipAPI ship, int frame, float amount) {
		if (proj instanceof MissileAPI) {
			MissileAPI mine = (MissileAPI)proj;
			if (mine.isMine() && mine.getUntilMineExplosion() < frame * amount) {
				return MathUtils.isWithinRange(ship, mine, mine.getMineExplosionRange());
			}
		}
		...
		float projSpeed = projVel.length() * amount;
		float shipSpeed = shipVel.length() * amount;
	...
	public static boolean willProjectileHitShipWithInSec(DamagingProjectileAPI proj, ShipAPI ship, int sec, float amount) {
		if (amount <= 0f) return false;
		return willProjectileHitShipWithInFrame(proj, ship, Math.round((float)sec / amount), amount);
	}
	...
	public static MarketAPI pickMarket(FactionAPI faction, boolean withPriority) {
		if (withPriority) return pickMarket(faction, "Taivassija", "GAB11_yuanjiang");
	...
	public static class I18nSection {
		...
		public static String buildPercent(int value) {
			return value + "%";
		}

		public static String buildPercent(float value) {
			return buildFactor(value) + "%";
		}

		public static String buildFactor(float value) {
			BigDecimal number = new BigDecimal(String.format("%.2f", value));
			return number.stripTrailingZeros().toPlainString();
		}

		public static String buildNumber(int value) {
			return String.valueOf(value);
		}

		public static String buildNumber(float value) {
			return buildNumber(Math.round(value));
		}
	}
}
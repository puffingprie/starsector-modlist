...
public class goat_flarelaunchergreen implements EveryFrameWeaponEffectPlugin {
	...
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		...
		if (weapon.getShip().getSystem().isActive()) {
			...
			if (timer < 0.1f) {
				...
				DamagingProjectileAPI tank = (DamagingProjectileAPI)engine.spawnProjectile(weapon.getShip(), null, "flarelauncher3", weapon.getFirePoint(0), weapon.getCurrAngle() + MathUtils.getRandomNumberInRange(-10, 10), weapon.getShip().getVelocity());

...
public class goat_FireSmokeDecoEffect implements EveryFrameWeaponEffectPlugin {
		...
		if (ship != null && ship.getSystem() != null) {
			if (ship.getSystem().isActive()) {
				...
				if (timer < 0.01f) {
					...
					//fire
					if (!doOnce && fireTimer <= FIRE_DURATION) {
						addHitParticles(engine, 25f, 18f, 7f, 12f, 0.15f, 8, new Color(255, 120, 50, 245), loc, vol, weapon.getCurrAngle());

					}

					//smoke
					engine.addSmokeParticle(loc, new Vector2f(vol.x + (float)(Math.cos(Math.toRadians(weapon.getCurrAngle())) * 40f) + (Misc.random.nextFloat() - 0.5f) * 2f * 10f, vol.y + (float)(Math.sin(Math.toRadians(weapon.getCurrAngle())) * 40f) + (Misc.random.nextFloat() - 0.5f) * 2f * 10f), 18f, 0.02f, 3f, new Color(255, 250, 250, 25));
					timer = 0f;
				}

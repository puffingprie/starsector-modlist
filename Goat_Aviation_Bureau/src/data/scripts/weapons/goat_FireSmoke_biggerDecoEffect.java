...
public class goat_FireSmoke_biggerDecoEffect implements EveryFrameWeaponEffectPlugin {
		...
		if (ship != null && ship.getSystem() != null) {
			if (ship.getSystem().isActive()) {
				...
				if (timer < 0.02f) {
					...
					//fire
					if (!doOnce && fireTimer <= FIRE_DURATION) {
						addHitParticles(engine, 20f, 18f, 16f, 20f, 0.25f, 10, new Color(255, 120, 50, 245), loc, vol, weapon.getCurrAngle());

					}

					//smoke
					engine.addSmokeParticle(loc, new Vector2f(vol.x + (float)(Math.cos(Math.toRadians(weapon.getCurrAngle())) * 50f) + (Misc.random.nextFloat() - 0.5f) * 2f * 10f, vol.y + (float)(Math.sin(Math.toRadians(weapon.getCurrAngle())) * 50f) + (Misc.random.nextFloat() - 0.5f) * 2f * 10f), 25f, 0.02f, 3f, new Color(65, 60, 60, 125));
					timer = 0f;
				}

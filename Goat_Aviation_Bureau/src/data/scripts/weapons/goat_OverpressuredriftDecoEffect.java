...
public class goat_OverpressuredriftDecoEffect implements EveryFrameWeaponEffectPlugin {
		...
		if (ship != null && ship.getSystem() != null) {
			if (ship.getSystem().isActive()) {
				...
				if (timer < 0.01f) {
					...
					//fire
					if (!doOnce && fireTimer <= FIRE_DURATION) {
						addHitParticles(engine, 8f, 15f, 9f, 12f, 0.15f, 4, new Color(115, 20, 150, 255), loc, vol, weapon.getCurrAngle());

						addHitParticles(engine, 34f, 25f, 7f, 12f, 0.15f, 16, new Color(255, 120, 50, 195), loc, vol, weapon.getCurrAngle());

					}

					//smoke
					engine.addSmokeParticle(loc, new Vector2f(vol.x + (float)(Math.cos(Math.toRadians(weapon.getCurrAngle())) * 40f) + (Misc.random.nextFloat() - 0.5f) * 2f * 10f, vol.y + (float)(Math.sin(Math.toRadians(weapon.getCurrAngle())) * 40f) + (Misc.random.nextFloat() - 0.5f) * 2f * 10f), 18f, 0.02f, 3f, new Color(165, 160, 160, 25));
					timer = 0f;
				}
			} else {
				doOnce = false;

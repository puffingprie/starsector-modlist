package data.missions.goat_kingsbattlo;
...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "Your impromptu battle line");
		api.setFleetTagline(FleetSide.ENEMY, "Unidentified Hegemony Fleet");
		
		// These show up as items in the bulleted list under 
		// "Tactical Objectives" on the mission detail screen
		api.addBriefingItem("Noman must survive.");
		api.addBriefingItem("The enemy may be fast, but you're faster.");
		api.addBriefingItem("Dividing and conquering may be the path to victory.");
		...
		if (!testMode) {
			api.addToFleet(FleetSide.PLAYER, "goat_flowertide_Assault", FleetMemberType.SHIP, "GOA Noman", true);
		...
		api.addToFleet(FleetSide.ENEMY, "vanguard_Strike", FleetMemberType.SHIP, false);
			...
			api.defeatOnShipLoss("GOA Noman");
		}
...
//			member.getCaptain().getStats().setSkillLevel(Skills.HELMSMANSHIP, 2);
	...
	//		member.setCaptain(person);
		...
		api.addObjective(minX + width * 0.6f + 2000, minY + height * 0.7f, "sensor_array");
		api.addObjective(minX + width * 0.7f + 2000, minY + height * 0.5f, "nav_buoy");
		api.addObjective(minX + width * 0.3f + 1000, minY + height * 0.6f, "nav_buoy");
		...
		api.addPlanet(0, 0, 50f, StarTypes.BLUE_GIANT, 500f, true);

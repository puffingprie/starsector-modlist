...
public class SWP_AnamorphicFlare {
...
//
//            Color fringeColorP = new Color(fringeColor.getRed(), fringeColor.getGreen(), fringeColor.getBlue(), SWP_Util.clamp255(alphaf));
//            Color coreColorP = new Color(coreColor.getRed(), coreColor.getGreen(), coreColor.getBlue(), SWP_Util.clamp255(alpha));
//
...
//    }
    public static void createStripFlare(ShipAPI origin, Vector2f point, CombatEngineAPI engine, float brightness, int magnitude, float thickness,
            float width, float angle, float spread, float fringeGain, Color fringeColor, Color coreColor, boolean singleFlicker) {
        ...
        for (int i = 0; i < magnitude; i++) {
            ...
            EmpArcEntityAPI arc = engine.spawnEmpArc(origin, point, new SimpleEntity(point), new SimpleEntity(location),
                    DamageType.ENERGY,
                    0.0f,
                    0.0f, // emp
                    1000f, // max range

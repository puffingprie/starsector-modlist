...
//import com.fs.starfarer.api.impl.campaign.fleets.TiandongAutofitDefs;
...
import data.missions.FEDMissionStationDefs.AvailableStation;
import data.campaign.fed_PersistentUnlocker;
...
public class BaseRandomFEDMissionDefinition implements MissionDefinitionPlugin {
    //public static Logger log = Global.getLogger(BaseRandomFEDMissionDefinition.class);
    ...
    protected static final Map<String, Float> QUALITY_FACTORS = new HashMap<>(39);
    ...
    protected static final List<String> TEST_HULLS = new ArrayList<>(Arrays.asList(new String[]{
        ...
        "fed_lightdestroyer",
        "fed_rigger",
        "fed_rigger_firesupport",
        "fed_tern",
        "fed_talos",
        "fed_byte",
        "fed_heavyfreighter", //"fake_hull_lol" // You know, to test if the error message works
    }));

    protected static final List<String> RARE_SHIPS = new ArrayList<>(Arrays.asList(new String[]{
        "fed_boss_rebel_standard",
        "fed_cargocap_standard",
        "fed_superdestroyer_pirate_standard",
        "fed_mantis_cruiser_standard",
        "fed_nisos_standard",
        "fed_shivan_standard",
        "fed_stormwalker_standard",
        "fed_polyp_standard",}));
    ...
    static {
        ...
        QUALITY_FACTORS.put("star_federation", 0.7f);       // Federation ships are up to spec, for the most part
    ...
    protected static boolean stationMode = false;
    // Enforce station battles
    ...
    // FED variant dump. Only works if variant test mode is also on
    protected static boolean variantTester = false;
    protected static boolean rareShip = false;
    // Speeds up battle
    ...
    public void defineMission(MissionDefinitionAPI api) {
        ...
        if (isCtrlKeyPressed(Keyboard.KEY_R)) {
            stationMode = !stationMode;

            if (stationMode) {
                Global.getSoundPlayer().playUISound("burst_pd_fire", 1f, 1f);
            } else {
                stationMode = false;
                Global.getSoundPlayer().playUISound("burst_pd_fire", 1f, 1f);
            }
        }
        if (isCtrlKeyPressed(Keyboard.KEY_S)) {
            testMode = !testMode;
        ...
        if (isCtrlKeyPressed(Keyboard.KEY_H)) {
            rareShip = !rareShip;
        ...
        if ((Math.random() < STATION_CHANCE) || stationMode) {
            // 0 = player, 1 = enemy, -1 = don't spawn a station
            int stationSide = Math.random() < STATION_CHANCE_PLAYER ? 0 : 1;
            if (stationSide == 0) {
                stationPlayer = FEDMissionStationDefs.getStationForFaction(player.getId(), size * difficulty * 2);
            } else if (stationSide == 1) {
                stationEnemy = FEDMissionStationDefs.getStationForFaction(enemy.getId(), size * 2);
            }
        }
        ...
        if (!(testMode && variantTester)) {
            ...
            api.addBriefingItem("VARIANT TESTING MODE: Listing all variants of Star Federation hulls");
        }
        ...
        if (testMode) {
            if (!variantTester) {
                api.addBriefingItem("TEST MODE enabled: Other mod factions now available and the Star Federation can now infight");
        ...
        if (speedMode) {
            api.addPlugin(new BaseEveryFrameCombatPlugin() {
                ...
                public void advance(float amount, List<InputEventAPI> events) {
                    ...
                    Global.getCombatEngine().getTimeMult().modifyMult("fed_tester", newTimeMult);
                }
            });
        }
    }
    ...
    protected void chooseFactions(String playerFactionId, String enemyFactionId) {
        if (testMode && variantTester) {
            player = Global.getSettings().createBaseFaction("star_federation");
            enemy = Global.getSettings().createBaseFaction("star_federation");
            return;
        }
        ...
        if (testMode || fed_PersistentUnlocker.knowDerelicts) {
            acceptableFactions.add(Global.getSettings().createBaseFaction(Factions.DERELICT));
        }
        if (testMode || fed_PersistentUnlocker.knowRemnants) {
    ...
    protected int generateFleet(FactionAPI faction, MissionDefinitionAPI api, FleetSide side, int fp, AvailableStation station) {
        ...
        if (rareShip && fp >= 30) {
            ...
            int sizeSelect = (int) MathUtils.getRandomNumberInRange(0f, RARE_SHIPS.size() - 1f);

            String secretShip = RARE_SHIPS.get(sizeSelect);

            api.addToFleet(FleetSide.PLAYER, secretShip, FleetMemberType.SHIP, true);
            fp -= api.getFleetPointCost(secretShip);
            rareShip = false;
        }
        ...
        for (FleetMemberAPI baseMember : fleetList) {
            ...
            //if (baseMember.getHullSpec().getHints().contains(ShipHullSpecAPI.ShipTypeHints.SHIP_WITH_MODULES)) {
            //}
...
protected void randomizeVariant(CoreAutofitPlugin auto, AutofitPlugin.AutofitPluginDelegate inflater, FleetMemberAPI member,
        ...
        if (member.getHullSpec().hasTag("no_autofit")) {
            return;
        }

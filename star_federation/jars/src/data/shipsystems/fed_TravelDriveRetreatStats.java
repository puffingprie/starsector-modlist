package jars.src.data.shipsystems;
...
public class fed_TravelDriveRetreatStats extends BaseShipSystemScript {
        ...
        if (engine.getCustomData().containsKey(ship.getFleetMemberId() + "_usedTravelDrive")) {
            if (engine.getCustomData().get(ship.getFleetMemberId() + "_usedTravelDrive").equals(true)) {
                if (ship.getFacing() == 90f) {
                    if (Global.getCombatEngine().getContext().getOtherGoal() != null) {
                        ...
                        } else if (Global.getCombatEngine().getTotalElapsedTime(false) > 10f

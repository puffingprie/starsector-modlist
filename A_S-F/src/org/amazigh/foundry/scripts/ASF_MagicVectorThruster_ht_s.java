// Base script by Tartiflette, Edited to remove the "thruster wiggle"
...
public class ASF_MagicVectorThruster_ht_s implements EveryFrameWeaponEffectPlugin {
    ...
    private float time=0, previousThrust=0, glowScalar = 0f;
    ...
    private Vector2f size= new Vector2f(10,68);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
    	...
    	Vector2f glowSize = new Vector2f(20f + (glowScalar * 10f), 20f + (glowScalar * 10f));
    	Vector2f glowLoc = MathUtils.getPointOnCircumference(weapon.getLocation(), 2f, weapon.getCurrAngle());
    	int alpha = 210;
    	if (weapon.getShip().getSystem().isActive()) {
    		double alphaTemp = alpha;
    		double timeMult = (double) weapon.getShip().getMutableStats().getTimeMult().modified;
    		alpha = (int) Math.ceil(alphaTemp / timeMult);				
    	}
    	
    	// from "VIC_EngineGlowScript" - we're colouring engines by script!
        ...
        Color glowColor = new Color(Red, Green, Blue, Math.min(255, alpha));
    	
    	MagicRender.singleframe(Glow, glowLoc, glowSize, 1f, glowColor, true);
    }
    ...
    private void thrust(WeaponAPI weapon, float angle, float thrust, float smooth){
        ...
        Color colorToUse = new Color(Red, Green, Blue, 255);

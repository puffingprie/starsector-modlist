...
// Modifications to make missiles with this script "swarm" outwards, use "MISSILE_SPREAD" behaviour, and have some random "jink" manoeuvres mid-flight 
...
public class ASF_PhiliaMissileAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    */
    ...
    private final int MAX_SEARCH_RANGE = 1600;
    ...
    private float PRECISION_RANGE=250;
    ...
    private float SWARM_OFFSET = 30f;
    
    // what the max angle offset for the Jink manoeuvres should be.
    private float JINK_OFFSET = 80f;
    
    // how strong the Jinks should be.
    private float JINK_POWER = 250f;

	// range under which the jinks start.
    private float JINK_RANGE=320;
    
    // "semi-random" timing for the Jinking.
    private IntervalUtil jinkInterval = new IntervalUtil(0.15f,0.25f);
    ...
    public ASF_PhiliaMissileAI(MissileAPI missile, ShipAPI launchingShip) {
        ...
        JINK_RANGE=(float)Math.pow((2*JINK_RANGE),2);
    ...
    public void advance(float amount) {
        ...
        // the first thing we do is the jink, (but only when within range) done first, to maybe have it play a little nicer with tracking.
        if (target != null) {
            if (MathUtils.getDistanceSquared(MISSILE.getLocation(), target.getLocation()) <= JINK_RANGE) {
            	
                jinkInterval.advance(amount);
        		if (jinkInterval.intervalElapsed()) {
                	
                	float jinkOffset = MathUtils.getRandomNumberInRange(-JINK_OFFSET, JINK_OFFSET);

...
public class RemnantSubsystemsUtil {
	...
	public boolean isMinCrewWithinAutomationLimit(ShipAPI ship) {
		return (ship.getVariant().getHullSpec().getMinCrew() <= AUTOMATION_MIN_CREW_UPPER_LIMIT);
	}
	
	//master method used by all adaptive hullmods to check for compatibility
	public boolean isApplicable(ShipAPI ship) {
		...
		if (isExperimental(ship)) return true; //Experimental Hullmods compatibility workaround
		else if (isAbomination(ship)) return false;
		if (isApplicableHull(ship)) return true;
	...
	public boolean isExperimental(ShipAPI ship) {
		...
		return (ship.getHullSpec().getManufacturer().equals("Experimental")
	...
	public boolean isAbomination(ShipAPI ship) {
		...
		if (!isMinCrewWithinAutomationLimit(ship)) return true;

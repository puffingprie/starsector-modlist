...
public class xlu_SteelPowerBurnStats extends BaseShipSystemScript {

	public static final float FLUX_BONUS = 0.33f;
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (state == ShipSystemStatsScript.State.OUT) {
			...
			stats.getAcceleration().modifyFlat(id, 20f * effectLevel);
                        
			stats.getMaxTurnRate().modifyMult(id, 1f - (0.75f * effectLevel));
			stats.getTurnAcceleration().modifyMult(id, 1f - (0.75f * effectLevel));
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		if (index == 1) return new StatusData("reduced turn rate", false);
                if (index == 2) return new StatusData("flux dissipation +" + (int) fluxDi + "%", false);
		return null;
	}
}

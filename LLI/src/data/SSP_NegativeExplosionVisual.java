...
public class SSP_NegativeExplosionVisual extends BaseCombatLayeredRenderingPlugin {
    ...
    protected SSP_NegativeExplosionVisual.SSP_NEParams p;
    public static class SSP_NEParams implements Cloneable {
        ...
        public Color underglow = Color.darkGray;
        public Color invertForDarkening = null;
        public int invertAlpha=127;
        public Color renderCircleColor = Color.black;
        public Color renderAtmosphereColor = Color.black;
        public SSP_NEParams() {
        }
        public SSP_NEParams(float radius, float thickness, Color color) {
            super();
            this.radius = radius;
        ...
        @Override
        public SSP_NEParams clone() {
            try {
                return (SSP_NEParams) super.clone();
            } catch (CloneNotSupportedException e) {
                return null; // should never happen
            }
        }
    }
    public SSP_NegativeExplosionVisual(SSP_NEParams p) {
        this.p = p;
    }
    ...
    public void advance(float amount) {
        ...
        if (!spawnedHitGlow && (!fader.isFadingIn() || fader.getBrightness() >= p.spawnHitGlowAt)) {
            ...
            Color c = new Color(255 - invert.getRed(),
                    255 - invert.getGreen(),
                    255 - invert.getBlue(), p.invertAlpha);
            ...
            for (int i = 0; i < 7; i++) {
                ...
                dur *= durMult;
                dur *= 0.5f;
                Vector2f pt = Misc.getPointAtRadius(point, size * 0.5f);
                ...
                engine.addNegativeNebulaParticle(pt, v, size, 2f,
                        p.fadeIn / dur, 0f, dur, new Color(c.getRed(),c.getGreen(),c.getBlue(),p.invertAlpha));
            }
            ...
            for (int i = 0; i < 15; i++) {
                ...
                engine.addNebulaParticle(loc, entity.getVelocity(), s, 1.5f, rampUp, 0f, dur, new Color(c.getRed(),c.getGreen(),c.getBlue(),p.invertAlpha));
            }
            spawnedHitGlow = true;
        }
    }
    ...
    public void render(CombatEngineLayers layer, ViewportAPI viewport) {
        ...
        if (layer == CombatEngineLayers.ABOVE_PARTICLES_LOWER) {
            ...
            renderCircle(x, y, r, circleAlpha, segments, p.renderCircleColor);
            renderAtmosphere(x, y, r, tCircleBorder, circleAlpha, segments, atmosphereTex, noise, p.renderAtmosphereColor, false);
        }
    }
